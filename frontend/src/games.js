...
function collector(stage,scoreEl,hint,finish){
 ...
 let touchStart=null; const ts=e=>{touchStart=e.changedTouches[0]}; const te=e=>{if(!touchStart)return;const q=e.changedTouches[0],dx=q.clientX-touchStart.clientX,dy=q.clientY-touchStart.clientY;if(Math.abs(dx)>Math.abs(dy))keys[dx>0?"d":"a"]=1;else keys[dy>0?"s":"w"]=1;setTimeout(()=>{keys.a=keys.d=keys.w=keys.s=0},120);touchStart=null};stage.addEventListener("touchstart",ts,{passive:true});stage.addEventListener("touchend",te,{passive:true});
 ...
 const timer=setInterval(()=>{hint.textContent=`${t--}s · WASD / Pfeile`;if(t<0){alive=false;finish(score)}},1000);cleanup=()=>{alive=false;clearInterval(sp);clearInterval(loop);clearInterval(timer);removeEventListener("keydown",kd);removeEventListener("keyup",ku);stage.removeEventListener("touchstart",ts);stage.removeEventListener("touchend",te)}
}
...
function runner(stage,scoreEl,hint,finish){
 let score=0,alive=true,jump=false,vy=0;const r=document.createElement("div");r.className="runner";stage.append(r);const floor=document.createElement("div");floor.className="floor";stage.append(floor);const kd=e=>{if((e.key===" "||e.key==="ArrowUp")&&!jump){jump=true;vy=-13}};addEventListener("keydown",kd);const tap=()=>kd({key:" "});stage.addEventListener("pointerdown",tap);const obs=[];const sp=setInterval(()=>{const o=document.createElement("div");o.className="obstacle";o.style.left="100%";stage.append(o);obs.push({el:o,x:100})},1200);const loop=setInterval(()=>{vy+=.65;let b=parseFloat(getComputedStyle(r).bottom)+vy;if(b<=45){b=45;vy=0;jump=false}r.style.bottom=b+"px";obs.forEach((o,i)=>{o.x-=.8;o.el.style.left=o.x+"%";if(o.x<-10){o.el.remove();obs.splice(i,1);score+=100;scoreEl.textContent=score}const a=r.getBoundingClientRect(),c=o.el.getBoundingClientRect();if(!(a.right<c.left||a.left>c.right||a.bottom<c.top||a.top>c.bottom)){alive=false;finish(score)}})},25);const timer=setInterval(()=>hint.textContent=`Score ${score} · SPACE / ↑ springen`,1000);cleanup=()=>{alive=false;clearInterval(sp);clearInterval(loop);clearInterval(timer);removeEventListener("keydown",kd);stage.removeEventListener("pointerdown",tap)}
}
